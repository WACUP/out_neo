...
inline int mode_index(Speakers spk)
{
	switch (spk.mask)
	{
		...
		case MODE_QUADRO:  return 3;
		case MODE_3_2:     return 4;
		case MODE_5_1:     return 5;
	}
	return 0;
}

inline int mode_from_index(int index)
{
	switch (index)
	{
		case 0:  return 0;
		case 1:  return MODE_1_0;
		case 2:  return MODE_2_0;
		case 3:  return MODE_QUADRO;
		case 4:  return MODE_3_2;
		case 5:  return MODE_5_1;
	}
	return 0;
}
...
void ConfigDlg::init_controls()
{
	...
	sprintf(ver2, ver1, "DirectX matrix audio output v1.63b");
...
void ConfigDlg::reload_state()
{
	...
	m_outMixer->get_Output(&out_spk, &asis);
	...
	// Input/output levels
	vtime_t time = m_outMixer->GetOutputTime();
...
void ConfigDlg::update_static_controls()
{
	...
	SendDlgItemMessage(hwnd, IDC_CMB_SPK,    CB_SETCURSEL, asis ? 0 : mode_index(out_spk), 0);
	SendDlgItemMessage(hwnd, IDC_CMB_FORMAT, CB_SETCURSEL, format_index(out_spk), 0);
	SendDlgItemMessage(hwnd, IDC_CMB_RATE,	 CB_SETCURSEL, rate_index(m_outMixer->getRate()), 0);
	...
	switch (control)
	{
		...
		case IDC_CMB_SPK:
			if (message == CBN_SELENDOK)
			{
				...
				m_outMixer->set_OutputMode(mode_from_index(ispk));
				update();
			}
			break;
